(function() {
  ...
  function takepicture() {
      ...
      if (width && height) {
          ...
          $.ajax({
              type:    "POST",
              url : "http://127.0.0.1:5000/process_image_real",
              dataType: "json",
              ...
              success: function(data) {
                    ...
                    if(count > 1){
                    console.log(result)
                     if(result.face_auth == "False")
                     M.toast({html: 'Face Authentication Failed'})
                     if(result.phone == "True")
                     M.toast({html: 'Phone Detected'})
                     if(result.mouth_open_val == "True" && result.mouth_open_conf > 65)
                     M.toast({html: 'Dont Talk'})
                     //head pose remained

                    }
                    else
                    M.toast({html: 'Multiple Face Detected'})
                //   setTimeout(function(){ takepicture(); }, 300); 
                    startup()
              },
